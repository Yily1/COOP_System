...
// Get this user's linked member profile (Full Name, Membership ID, Type)
$stmt = $pdo->prepare("
    SELECT m.membership_id, m.last_name, m.first_name, m.middle_name, m.membership_type, m.farmer_type
    FROM users u
...
<div class="card">
    ...
    <p style="color: #666; margin-top: 10px; font-size: 16px;">
        <strong>Welcome, <?php echo ($myProfile && $myProfile['membership_id']) ? htmlspecialchars($myProfile['last_name'] . ', ' . $myProfile['first_name']) : htmlspecialchars($_SESSION['email']); ?>!</strong>
    </p>
    <table style="margin-top: 15px;">
        <?php if ($myProfile && $myProfile['membership_id']): ?>
        <tr>
            <th style="width: 200px; font-weight: normal;">Membership ID</th>
            <td><?php echo htmlspecialchars($myProfile['membership_id']); ?></td>
        </tr>
        <tr>
            <th style="font-weight: normal;">Type of Farmer</th>
            <td><?php echo htmlspecialchars($myProfile['farmer_type'] ?? '-'); ?></td>
        </tr>
        <tr>
            <th style="font-weight: normal;">Membership Type</th>
            <td><?php echo htmlspecialchars($myProfile['membership_type']); ?></td>
        </tr>
        <?php else: ?>
        <tr>
            <td colspan="2" style="color: #666;">No member profile linked to this account yet.</td>
        </tr>
        <?php endif; ?>
        <tr>
            <th style="width: 200px; font-weight: normal;">Email</th>
            <td><?php echo htmlspecialchars($_SESSION['email']); ?></td>
        </tr>
        <tr>
            <th style="font-weight: normal;">Role</th>
            <td><span class="badge badge-user">User</span></td>
        </tr>
    </table>
    <?php if ($myProfile && $myProfile['membership_id']): ?>
        <p style="margin-top: 15px;">
            <a href="<?php echo BASE_URL; ?>/app/users/user-view.php?user_id=<?php echo $userId; ?>">View Full Information</a>
        </p>
    <?php endif; ?>
</div>
